...
public class SysPluginService : IDynamicApiController, ITransient
{
    ...
    /// <param name="input"></param>
    /// <returns></returns>
    [DisplayName("获取动态插件分页列表（高级查询）")]
    public virtual async Task<SqlSugarPagedList<SysPlugin>> PageAdvanced(PageAdvancedInput input)
    {
        var query = _sysPluginRep.AsQueryable()
