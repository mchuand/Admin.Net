<template>
	<div class="sys-exlog-container">
		<el-card class="full-table" header-class="card_header" shadow="hover" style="margin-top: 5px">
			<template #header>
				<!-- 按钮栏组件 -->
				<ButtonBar mode="sysExlog" :buttonConfig="logButtonConfig" displayStyle="inline"
					:onButtonClick="handleButtonClick" />

				<!-- 高级查询组件 -->
				<AdvancedSearch ref="searchRef" :fields="searchFields" :keywordFields="keywordFields"
					mode="sysExlog" :disableAutoQuery="true" @query="handleAdvancedQuery" @reset="handleAdvancedReset" />
			</template>
...
const handleAdvancedQuery = async (conditions: QueryCondition[]) => {
	...
	try {
		let res = await getAPI(SysLogExApi).apiSysLogExPageAdvancedPost(params as any);
...
const exportLog = async () => {
	state.loading = true;
	var res = await getAPI(SysLogExApi).apiSysLogExExportPost({}, { responseType: 'blob' });
	state.loading = false;
